...
AS 
   ...
   */
   ...
   SELECT 
      ...
      T.REQUESTING_AGENCY, 
      T.REQUESTOR, 
      T.REQUESTOR_POSITION, 
      T.TASK AS TASK_NAME, 
      T.TASK_TYPE, 
      T.DUE_TIME AS DUE_DATE_TIME, 
      T.TASK_CATEGORY AS CATEGORY, 
      RR.RELATED_REPORT_NAME, 
      T.EXTRA_TASK_DETAIL AS ADDL_TASK_DETAILS, 
      T.PROGRESS, 
      T.CONCERNS, 
      T.APPROVEDBY, 
      T.APPROVEDTIME, 
      ...
      CASE 
         WHEN UC.USER_NAME IS NULL THEN 
            (
               ...
               WHERE TBL_REMOTE_DOCUMENT_USER_INFO.GLOBAL_REPORT_ID = T.TASK_ID
            )
         ELSE UC.USER_NAME
      END AS CREATED_BY, 
      ...
      CASE 
         WHEN UM.USER_NAME IS NULL THEN 
            (
               ...
               WHERE TBL_REMOTE_DOCUMENT_USER_INFO$2.GLOBAL_REPORT_ID = T.TASK_ID
            )
         ELSE UM.USER_NAME
      END AS MODIFIED_BY, 
   ...
   FROM 
      (EMBCPROD.TBL_TASK_STATUS_REPORT  AS T 
         LEFT JOIN EMBCPROD.TBL_PICKLIST_VALUE  AS PV 
         ON PV.PICKLIST_VALUE_ID = T.PRIORITY) 
         LEFT JOIN EMBCPROD.TBL_PICKLIST_VALUE  AS PV2 
         ON T.STATUS LIKE PV2.PICKLIST_VALUE_ID, 
      (EMBCPROD.TBL_REPORT  AS REPT 
         ...
         ON REPT.GLOBAL_REPORT_ID = RR.GLOBAL_REPORT_ID 
   ...
   WHERE T.TASK_TYPE = 'T' AND T.TASK_ID = REPT.GLOBAL_REPORT_ID
GO
EXECUTE sp_addextendedproperty @name = N'MS_SSMA_SOURCE', @value = N'EMBCPROD.V_TASK', @level0type = N'SCHEMA', @level0name = N'EMBCPROD', @level1type = N'VIEW', @level1name = N'V_TASK';
