create function [ssma_oracle].[get_value_blob] (@name nvarchar(323)) returns varbinary(max)
as begin
...
return (select b_value from ssma_oracle.db_storage with (READCOMMITTEDLOCK) where spid=@get_active_spid and login_time=@get_active_login_time and name=@name)
