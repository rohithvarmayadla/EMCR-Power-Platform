create function [ssma_oracle].[get_pv_clob] (@owner sysname, @package nvarchar(32), @name nvarchar(128)) returns varchar(max)
as begin
...
return (select c_value from ssma_oracle.db_storage with (READCOMMITTEDLOCK) where spid=@get_active_spid and login_time=@get_active_login_time and name = @owner + '.' + @package + '$' + @name)
end