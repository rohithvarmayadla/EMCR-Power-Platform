create function [ssma_oracle].[get_value_clob] (@name nvarchar(323)) returns varchar(max)
as begin
...
return (select c_value from ssma_oracle.db_storage with (READCOMMITTEDLOCK) where spid=@get_active_spid and login_time = @get_active_login_time and name=@name)
