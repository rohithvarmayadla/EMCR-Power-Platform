create function [ssma_oracle].[get_pv_xml] (@owner sysname, @package nvarchar(32), @name nvarchar(128)) returns xml
as begin
...
return cast((select nc_value from ssma_oracle.db_storage with (READCOMMITTEDLOCK) where spid=@get_active_spid and login_time=@get_active_login_time and name = @owner + '.' + @package + '$' + @name) as xml)
