create function [ssma_oracle].[get_pv_varchar] (@owner sysname, @package nvarchar(32), @name nvarchar(128)) returns varchar(8000)
as begin
...
return cast((select v_value from ssma_oracle.db_storage with (READCOMMITTEDLOCK) where spid=@get_active_spid and login_time=@get_active_login_time and name = @owner + '.' + @package + '$' + @name) as varchar(8000))
