create procedure [ssma_oracle].[set_pv_xml] (@owner sysname, @package nvarchar(32), @name nvarchar(128),@value xml)
as begin
...
declare @casted_value nvarchar(max)
set @casted_value = cast(@value as nvarchar(max))
if @@trancount=0 begin
 declare @full_name nvarchar(323)
 set @full_name = @owner + '.' + @package + '$' + @name
 update ssma_oracle.db_storage
 set c_value=null,v_value=null,nc_value=@casted_value,b_value=null
 ...
 values(@get_active_spid,@get_active_login_time,@full_name,null,null,@casted_value,null)
end else begin
 declare @dbname sysname 
  set @dbname = db_name()
 exec master..xp_ora2ms_exec2 @get_active_spid,@get_active_login_time,@dbname,'ssma_oracle','set_pv_xml',@owner,@package,@name,@value
end
end