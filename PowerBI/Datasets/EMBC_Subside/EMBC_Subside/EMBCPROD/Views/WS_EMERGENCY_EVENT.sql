...
CREATE VIEW [EMBCPROD].[WS_EMERGENCY_EVENT] (
   id, 
   currentStatus, 
   currentStatusValue, 
   prognosis, 
...
AS 
   SELECT 
      ...
      EMBCPROD.GET_RELATED_REPORT_FOR_EVENT(REPT.GLOBAL_REPORT_ID) AS relatedReportName, 
   ...
   WHERE EV.REPORT_TYPE = 'E' AND EV.EVENT_ID = REPT.GLOBAL_REPORT_ID

GO
EXECUTE sp_addextendedproperty @name = N'MS_SSMA_SOURCE', @value = N'EMBCPROD.WS_EMERGENCY_EVENT', @level0type = N'SCHEMA', @level0name = N'EMBCPROD', @level1type = N'VIEW', @level1name = N'WS_EMERGENCY_EVENT';
