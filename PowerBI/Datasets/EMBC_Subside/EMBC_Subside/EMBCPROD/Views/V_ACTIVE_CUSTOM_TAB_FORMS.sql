...
CREATE VIEW [EMBCPROD].[V_ACTIVE_CUSTOM_TAB_FORMS] (
   FREVVO_ID, 
   GLOBAL_REPORT_ID, 
   FORM_NAME, 
   INSTANCE_ID, 
...
AS 
   ...
   WHERE 
      ...
      TC.FREVVO_XML_DATA IS NOT NULL AND 
      CFR.REPORT_TYPE_ID IS NOT NULL AND 
      REPT.STATUS IN ( 'A', 'C', 'D' )

GO
EXECUTE sp_addextendedproperty @name = N'MS_SSMA_SOURCE', @value = N'EMBCPROD.V_ACTIVE_CUSTOM_TAB_FORMS', @level0type = N'SCHEMA', @level0name = N'EMBCPROD', @level1type = N'VIEW', @level1name = N'V_ACTIVE_CUSTOM_TAB_FORMS';
