CREATE PROCEDURE [EMBCPROD].[DUTYLOG_LOG_MSG_SITSUM]  
AS 
   BEGIN
          ...
          C1 CURSOR LOCAL FOR 
            SELECT 
               ...
               CASE 
                  ...
                  ELSE NULL
               END
            FROM ((((EMBCPROD.TBL_DUTY_LOG  AS RES 
               LEFT JOIN EMBCPROD.TBL_REPORT  AS REPT 
               ON RES.DUTY_LOG_ID = REPT.GLOBAL_REPORT_ID) 
            ...
            WHERE 
               REPT.STATUS IN ( 'A', 'D' ) AND 
               REPT.SITSUM_ID_LIST IS NULL AND 
               NOT (RES.LOG_MESSAGE IS NULL)
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_SSMA_SOURCE', @value = N'EMBCPROD.DUTYLOG_LOG_MSG_SITSUM', @level0type = N'SCHEMA', @level0name = N'EMBCPROD', @level1type = N'PROCEDURE', @level1name = N'DUTYLOG_LOG_MSG_SITSUM';
